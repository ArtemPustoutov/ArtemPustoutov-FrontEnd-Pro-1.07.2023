...
let orderConteiner = document.createElement('div');
let clearbtn = document.createElement('button')
clearbtn.textContent = 'Delate'
...
let  ul = `
    <ul>
        ...
        <li><button class="order" id="my-orders">My orders</button></li>
    </ul>
`;
    ...
    button.addEventListener('click', () => {
        ...
        productNames.forEach(name => {
            ...
            buyButton.addEventListener('click', () => {
                ...
                localStorage.setItem(`${name} ${currentTime} `, JSON.stringify(productInfo));//Добавляем в LocalStorage
                clear()
            });
...
let showOrders = function() {
    ...
    for (let i = 0; i < localStorage.length; i++) {
        ...
        orderConteiner.innerHTML += `<p>${key}: ${localStorage.getItem(key)}</p>`;//Выводим при нажатии на кнопку My orders
        document.body.append(orderConteiner)
    }
    orderConteiner.appendChild(clearbtn);
}

let cleraCart = function() {
    orderConteiner.textContent = ''
    localStorage.clear()                           //очищаем корзину и локал Сторедж
}

let order = document.getElementById('my-orders');
order.addEventListener('click', showOrders)
clearbtn.addEventListener('click', cleraCart)